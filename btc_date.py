...
# Function to trace a Bitcoin transaction and its inputs, now including the date
def trace_transaction(tx_hash, depth=3):
    ...
    # Convert the UNIX timestamp to human-readable date
    if 'confirmed' in transaction:
        transaction_date = datetime.datetime.fromtimestamp(transaction['confirmed']).strftime('%Y-%m-%d %H:%M:%S')
    else:
        transaction_date = "Pending / Unconfirmed"
    
    # Display basic transaction information with the date
